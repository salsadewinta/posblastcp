...
const loadPelanggan = async (req, res) =>{
  ...
  try{
      ...
      res.render('loket/pelanggan/get', {user: req.session.user, infos:infos, infos: reversedPosts });
  }
...
const loadUpdatePelanggan = async (req, res) =>{
  ...
  try{
      ...
      res.render('loket/pelanggan/update', {user: req.session.user, infos:infos });
  }
...
const updatePelanggan = async(req, res) => {
  ...
  const {
      ...
      const approval = req.file ? `/images/${req.file.filename}` : null; 
  Info.findByIdAndUpdate(id, {
    ...
    .then(() => {
      res.redirect('/loket/pelanggan/get');
    })
...
const loadCreate = async (req, res) => {
  try {
    ...
    res.render('loket/pelanggan/create', {
...
const createPelanggan = async (req, res) => {
  try {
    // Generate a custom id for pelanggan
    ...
    // Format phone number
    const tlp = req.body.tlp.replace(/[^+\d]/g, ''); // Remove any character except + and digits

    // Format date
    const rawDate = req.body.tgl_transaksi;
    const tgl_transaksi = new Date(rawDate).toISOString().split('T')[0]; // Only get the date part

    // Handling the uploaded file for approval (image)
    const approval = req.file ? `/images/${req.file.filename}` : null;

    // Prepare the data object to be inserted
    const newPelanggan = new Info({ 
      id_pelanggan, 
      nama, 
      tlp, 
      jenis_transaksi, 
      ...
      kota, 
      provinsi, 
      status: 'belum',  // Set default status
      approval, 
      dokumentasi 
    });
...
const loadUjipetik = async (req, res) =>{
  ...
  try{
      var infos = await Info.find(id)
      const reversedPosts = infos.reverse();
      res.render('loket/pelanggan/ujipetik', {user: req.session.user, infos:infos, infos: reversedPosts });
  }
...
const updateUjipetik = async(req, res) => {
  ...
  Info.findByIdAndUpdate(id, {
    ...
    .then(() => {
      res.redirect('/loket/pelanggan/ujipetik');
    })
    .catch(err => console.log(err));
}
...
const loadUpdateUjipetik = async (req, res) =>{
  ...
  try{
      const infos = await Info.findById({ _id: id });
      res.render('loket/pelanggan/update_ujipetik', {user: req.session.user, infos:infos });
  }
